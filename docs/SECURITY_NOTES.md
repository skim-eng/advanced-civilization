...
| SEC-006 | High | Blocked before Phase 2 deployment | Root Pages middleware implements CSP/frame/HSTS/noindex/referrer/nosniff/Permissions-Policy and API no-store; wildcard CORS was removed and write origins are exact. Access cannot be activated without authorizing possible overage card charges, which was not approved, so no public deployment ran. Provider rate controls and hosted checks remain pending. |
