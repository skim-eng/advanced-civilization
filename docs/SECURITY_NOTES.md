...
| SEC-003-RLS | Critical confidentiality | Provider verification passed | Hosted PostgREST returns 401 for anon and 403 for authenticated CRUD; service-role lifecycle/targeted cleanup passes. Hosted Realtime sends only `{turn}`/empty signals and publishes no `dbf_*` tables. Deployed browser refetch remains pending. |
...
| SEC-006 | High | Blocked before Phase 2 deployment | Root Pages middleware implements CSP/frame/HSTS/noindex/referrer/nosniff/Permissions-Policy and API no-store; wildcard CORS was removed and write origins are exact. Official Cloudflare docs and account UI show no active/no-card/hard-cap Access path: activation requires recurring overage-charge authorization and budget alerts do not cap usage. No consent or deployment occurred. Provider rate controls and hosted checks remain pending. |
