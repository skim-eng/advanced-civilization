...
|---|---|---|---|---|---|---|---|---|
| SEC-001 / KI-002 | UI and server contain a fixed upstream hub URL for splash/cross-promotion, counters, identity, ratings, and leaderboard traffic. | Player, game, and request metadata can leave the fork without owner approval. | `src/ui/App.tsx`, `src/ui/main.tsx`, `src/ui/online.tsx`, `src/server/game-server.ts`, `functions/api/[[path]].ts` | Remove upstream defaults; permit optional services only through explicit default-off flags and owner-controlled endpoints; document the network allowlist. | Browser and server request capture proves zero unexpected egress while local multiplayer remains functional. | Pending | `npm test && npm run test:e2e` | BLOCKED |
