...
| SEC-003 / KI-004 | Canonical ordered SQL now includes every framework 0.42 field. A fresh persistent PGlite/PostgreSQL cluster passes create/seats/projections/move/message/report-disabled/restart/reconnect/continue/delete/purge. Hosted verification remains pending. | Fresh databases reproduce the actual server lifecycle without manual drift. | `supabase/schema.sql`, `supabase/migrations/*`, schema tests, `docs/SCHEMA_MIGRATIONS.md` | Retain migration ordering and snapshot equality; repeat through owner Supabase only in Phase 2. | Fresh migration plus complete lifecycle, restart, cleanup, and required-column assertion. | Pending | `npm run test:schema` | IN PROGRESS |
| SEC-003-RLS | Clean PostgreSQL tests prove all four RLS flags, no public policies, anon/authenticated SELECT/INSERT/UPDATE/DELETE denial, and service-role lifecycle. Canary scans find no server secret names/values in browser artifacts. Hosted Realtime service verification remains Phase 2. | Browser roles cannot directly read tokens, snapshots, messages, or reports. | migrations, RLS test, browser-artifact scanner, CI, `docs/SCHEMA_MIGRATIONS.md` | Keep all table access server-only and repeat PostgREST/Realtime checks before deployment. | Browser-role denial; privileged lifecycle; built-output/source-map scan; state-free broadcast proof. | Pending | `npm run test:rls && npm run test:secrets` | IN PROGRESS |
...
| SEC-011 / KI-011 | Node/Pages enforce JSON content type, a 64 KiB stream cap, structural/field limits, strict endpoint shapes, revision-aware moves, random request IDs, and fixed safe errors. Local malformed/stale/duplicate/race/failure tests pass; hosted verification remains. | Oversized or malformed requests fail before mutation; stale/duplicate writes cannot replay a turn; backend secrets are not serialized. | Node/Pages body readers, router validators, client move API, `docs/API_SECURITY.md`, tests | Retain strict schemas and safe-error mapping; include the full negative matrix in hosted CI. | Malformed/empty/oversize/unknown/nested/stale/duplicate/raced/wrong-seat/failure/interruption/error-serialization cases. | Pending | `npm test -- src/server && npm run test:e2e` | IN PROGRESS |
