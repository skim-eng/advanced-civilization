...
| KI-016 | Blocked before Phase 2 deployment | Response controls and fail-closed Functions are implemented locally. Access activation requires explicit authorization for possible overage card charges; the owner has not approved it, so no deployment ran. The selected Cloudflare account has no `kimsvideo.org` zone; custom-domain work is not applicable. Backup/restore, rollback, rate control, and hosted origin checks remain pending. |
